...
# cd /home/young/Kaggle_histopathologic-cancer-detection/my_mode_new_architecture/prj_root/src/utils && \
# rm e.l && python utils_split_files_into_directories_to_easily_upload_files_onto_colab.py \
# 2>&1 | tee -a e.l && code e.l

"""
This file can run separately to process big data files into separated directories for the purpose of
1. you can see and access image files easily from separated directories
2. you can upload big data files onto Google colaboratory easily
And after uploading zip files onto Google colaboratory, 
I use 3rd party app called ZIP Extractor to decompress zip files
3. or you can directly upload each directory (like /00, /01) onto Google colab without using zip files
"""
...
def create_text_files_which_contains_all_paths_of_image_files():
  trn_dir={"trn_txt_file":"/mnt/1T-5e7/mycodehtml/bio_health/Kaggle_histopathologic-cancer-detection/Data/train"}
...
def remove_useless_paths_from_text_file_containing_split_paths():
  trn_txt_file_path="/mnt/1T-5e7/mycodehtml/bio_health/Kaggle_histopathologic-cancer-detection/Data/train_split/trn_txt_file.txt"
  # Load text file into list
  lines_list=[]
  with open(trn_txt_file_path) as f:
    lines=f.readlines()
...
# remove_useless_paths_from_text_file_containing_split_paths()
